...
function buildWorkflow(workflowJson, subs = {}) {
  ...
  // Optional: %seed% is replaced with a fresh random integer (unquoted) so
  // repeated generations don't return the same (or ComfyUI-cached) image.
  if (filled.includes('%seed%')) {
    const seed = Math.floor(Math.random() * 1_000_000_000_000_000);
    filled = filled.split('%seed%').join(String(seed));
  }
  try {
